...
- When on an AMBOSS question page, click the extension icon in the toolbar
- Pick a skill (Causal Explainer or USMLE Flowchart)
- The extension scrapes the question, answers, explanation, attending tip, and (if applicable) your wrong first choice + its explanation
- It opens a new claude.ai tab and pastes everything with the skill slash command prefix, then hits enter
- Claude generates the diagram using the skill you selected
- You can open tabs in the background (toggle in the popup) so you don't leave AMBOSS

# Known limitations:
- **Claude.ai concurrent generation limit (max 3 active tabs):** Claude.ai only allows 3 tabs to be actively generating at the same time. The extension will still scrape and paste into a 4th tab and hit enter, but Claude will show a "too many active tabs" popup and refuse to start generating. You need to wait for at least one of the 3 active generations to finish before triggering another.
- **Claude.ai selectors may change:** The extension injects text into Claude's ProseMirror editor using DOM selectors. If Anthropic updates their website UI, the injection may break and selectors in config.js will need updating.

# Customization:
- **Prompt templates** can be edited in the debug page (click "Debug" link in the popup). Templates use {{placeholder}} syntax with {{#if field}}...{{/if}} conditionals.
- **Skills** are defined in config.js -- add new ones by adding entries to the SKILLS array with an id, label, and slash command prefix.
